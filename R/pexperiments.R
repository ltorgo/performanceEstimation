...
## ==============================================================
##
pperformanceEstimation <- function(tasks,workflows,estTask,...) {
  ...
  allRes <- foreach(d=1:length(tasks)) %dopar% {
#  for(d in 1:length(tasks)) {
      ...
      taskRes <- foreach(s=1:length(workflows)) %dopar% {
...
##                      CvTask(1,10,1234))
##
pcvEstimates <- function(wf,task,estTask) {
  ...
  itsInfo <- foreach(it=1:nits,.packages="randomForest") %dopar% {
