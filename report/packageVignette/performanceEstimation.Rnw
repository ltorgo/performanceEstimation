...
opts_chunk$set(cache.path='cache/perfEst-',dev='pdf',fig.path='figures/perfEst-', fig.align='center', tidy=FALSE,size="footnotesize",message=FALSE,cache=TRUE)
...
<<ex1Iris,fig.cap="The distribution of the error rate on the 10 folds. Red dots show the performance on each individual iteration of the estimation process.",out.width="0.7\\textwidth">>=
plot(res)  
@ 
...
dat <- as.data.frame(modelData(data.model))
colnames(dat)[1] <- 'PercVarClose'
spExp <- performanceEstimation(
  PredTask(PercVarClose ~ .,dat,'SP500_2012'),
...
<<cdn,out.width="\\textwidth",fig.width=10,fig.height=6,size="scriptsize",fig.cap="The CD diagram of the Nemenyi post-hoc test.">>=
