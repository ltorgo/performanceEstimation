...
opts_chunk$set(cache.path='cache/perfEst-',dev='pdf',fig.path='figures/perfEst-', fig.align='center', fig.show='hold',tidy=FALSE,size="footnotesize",message=FALSE,cache=TRUE)
options(replace.assign=TRUE,width=90)
##
## Load the package (later this should be substituted by library(performanceEstimation))
##library(devtools)
...
<<ex1Iris,fig.cap="The distribution of the error rate on the 10 folds. Red dots show the performance on each individual iteration of the estimation process.",out.width="0.8\\textwidth">>=
plot(res)  
@ 
...
<<standardWF_ex2,results="hide",cache=TRUE>>=
data(Boston,package='MASS')
library(DMwR)
library(e1071)
library(randomForest)
...
<<slideAndGrowEx, results="hide",cache=TRUE>>=
library(performanceEstimation)
library(quantmod)
library(randomForest)
...
<<RLensembleExp,results="hide",eval=FALSE>>=
data(algae,package='DMwR')
...
<<userDef-eval>>=
powErr <- function(itResults,stats,pow=3) {
    if (stats != "pow.err") stop("Unable to calculate that metric!")
    c(pow.err = mean((itResults$trues-itResults$preds)^pow))
}
@
...
<<cdn,out.width="0.9\\textwidth",fig.width=10,fig.height=6,size="scriptsize",fig.cap="The CD diagram of the Nemenyi post-hoc test.">>=
...
<<maeA1,fig.cap="The MAE results for the task ``a1''.",out.width="0.75\\textwidth">>=
...
<<mseA1b,fig.cap="Illustration of the use of regular expressions in sub-setting the results objects.",out.width="0.75\\textwidth">>=
