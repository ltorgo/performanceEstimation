...
opts_chunk$set(cache.path='cache/perfEst-',dev='pdf',fig.path='figures/perfEst-', fig.align='center', fig.show='hold')
...
RLensemble <- function(f,tr,ts,weightRT=0.5,step=FALSE,...) {
  ...
  c(correlation=cor(responseValues(f,ts),ps))
}
@ 
...
spExp <- performanceEstimation(
  ...
  c(Workflow('standardWF',wfID="standRF",
    ...
    Workflow('timeseriesWF',efID,"slideRF",
             learner='randomForest',learner.pars=list(ntree=500,relearn.step=5))
   ),
