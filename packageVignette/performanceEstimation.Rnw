...
opts_chunk$set(cache.path='cache/perfEst-',dev='pdf',fig.path='figures/perfEst-', fig.align='center', fig.show='hold',tidy=FALSE)
...
<<iris_small_ex, eval=FALSE, size="footnotesize">>=
...
<<ex1Iris,fig.cap="The distribution of the error rate on the 10 folds.",out.width="0.7\\textwidth">>=
plot(res)
@ 
...
myWorkFlow <- function(form,train,test,..., 
  ...
  res <- WFoutput(scores)
  if (.outPreds) 
      workflowPredictions(res) <- list(responseValues(form,test),
                                       preds,
                                       rownames(test))
...
<<eval=FALSE>>=
workflowVariants('myWorkFlow',x=c(0,3,5,7),y=c(T,F))
@ 
...
<<eval=FALSE>>=
workflowVariants('myWorkFlow',x=c(0,3,5,7),y=c(T,F),as.is=c('x'))
@ 
...
<<standardWF_ex2,eval=FALSE,size="small">>=
data(Boston,package='MASS')
library(DMwR)
library(e1071)
library(randomForest)
bostonRes <- performanceEstimation(
  PredTask(medv ~ .,Boston),
  c(workflowVariants('standardWF',learner='rpartXse',
...
bcExp <- performanceEstimation(
  ...
  workflowVariants('standardWF',
           learner='svm',
           learner.pars=list(cost=c(1,5),gamma=c(0.01,0.1)),
           evaluator.pars=list(stats=c("F","prec","rec"),
                               posClass="malignant")
          ),
...
spExp <- performanceEstimation(
  ...
  c(Workflow('standardWF',wfID="standRF",
    ...
    Workflow('timeseriesWF',efID,"slideRF",
...
<<>>=
pairedComparisons(ltrExp,'rpartXse.v1')
@ 
